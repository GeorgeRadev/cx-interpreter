...
public class TestParserExpressions extends TestCase {
	...
	public void testTernaryExpressions() {
		...
		NodeBlock block;
		{
			...
			block = parser.parse();
			NodeTernary ternary = (NodeTernary) block.statements.get(0);
		...
		}
		{
			...
			block = parser.parse();
			NodeBinary binary = (NodeBinary) block.statements.get(0);
	...
	public void testUnaryExpressions() {
		...
		NodeBlock block;
		{
			parser = new Parser("+p;");
			block = parser.parse();
			NodeVariable var = (NodeVariable) block.statements.get(0);
		...
		}
		{
			parser = new Parser("-p;");
			block = parser.parse();
			NodeUnary preInc = (NodeUnary) block.statements.get(0);
	...
	public void testCardinalExpressions() {
		...
		NodeBlock block;
		{
			...
			block = parser.parse();
			NodeNumber var = (NodeNumber) block.statements.get(0);
		...
		}
		{
			...
			block = parser.parse();
			NodeString var = (NodeString) block.statements.get(0);
